...
public class NewTenantValidator implements Validator{
	...
	Logger logger=LoggerFactory.getLogger(NewTenantValidator.class);
	...
	public void validateTenant(NewTenantForm newTenantForm, Errors errors){
		...
		if(newTenantForm.getMobile().isEmpty() || newTenantForm.getMobile() == null){
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "mobile", "required.mobile", "Please Enter Mobile Number.");
		}else{
...
//			Matcher matcher=pattern.matcher(newTenantForm.getEmail());
//			if(!matcher.matches()){
//				errors.rejectValue("id", "id.incorrect","Please Enter Valid Gov. Id.");
//			}
		}
